...
| 入场限制 | 24h回调须在 (2%, 5%]：≤2% 不够 / >5% 跌太多，均不开空 |
| 止损 | **无**（stoploss=-99.0，配合10x杠杆需反向990%才触发） |
| 止盈 | 移动止盈：均价偏离 **4%** 激活，从极值回撤 **2%** 平仓 |
| 超时 | 18 小时后价格回到成本价平仓 |
| DCA#1 | 首仓 **+10%** 涨幅触发（亏损中加仓） |
| DCA#2+ | 基于上次加仓价，先涨 **50%/40%** 再回调 **20%** 触发（可配置） |
| 资金费率 | < **-0.07%** 仅禁止开空，DCA加仓不受限制 |
